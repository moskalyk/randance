...
const executeTx = async (walletAddress: string, ethAuthProofString: any) => {
    ...
    if(!isValid) throw new Error('invalid wallet')
    ...
    const erc721Interface = new ethers.utils.Interface([
        'function relayClaim(address sequenceWallet_) public'
    ])

    const data = erc721Interface.encodeFunctionData(
        'relayClaim', [walletAddress]
    )
